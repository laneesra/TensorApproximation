...
from cnn import TTConvLayer, TTLayer, LinearLayer
...
class Decomposer:
    ...
    @timeit
    def replace_layer(self, keys=['0'], type='conv'):
        logger.info('===========before============')
        ...
        if type == 'conv':
            ...
            for key in keys:
                if isinstance(self.model.classifier._modules[key], torch.nn.modules.linear.Linear):
                    ...
                    decomposed[key] = self.decompose_fc_layer(fc_layer)
    ...
    def decompose_conv_layer(self, layer):
        ...
        logger.info('before shape', weights.shape)
        ...
        if self.factorization == 'cp':
            ...
            if self.verbose:
                ...
                for i in range(rk):
                    ...
                    decomposed += tmp
                logger.info('=========error============')
                print('tt-svd error', weight_tensor.frobenius_norm(weight_tensor.T - decomposed))
                logger.info(weight_tensor.relative_error(decomposed))
            ...
            f_x = f_x * lambdas
            logger.info('after shape', f_s.shape, f_y.shape, f_x.shape, f_t.shape)
        ...
        elif self.factorization == 'cp_tpm':
            rk = max(weight_tensor.T.shape) // 3
            lambdas, Us = weight_tensor.cp_rand(rk, init='random', ret_tensors=True)
            ...
            k_xy_layer = torch.nn.Conv2d(in_channels=rk, out_channels=rk, kernel_size=d,
            ...
            sr = torch.tensor(f_s).t_().unsqueeze_(-1).unsqueeze_(-1)
            rt = torch.tensor(f_t).unsqueeze_(-1).unsqueeze_(-1)
            rr = torch.stack(
                [torch.tensor(f_y).narrow(1, i, 1) @ torch.t(torch.tensor(f_x)).narrow(0, i, 1) for i in range(rk)]).unsqueeze_(1)

            k_s_layer.weight.data = sr.float()
            k_xy_layer.weight.data = rr.float()
            k_t_layer.weight.data = rt.float()

            new_layers = [k_s_layer, k_xy_layer, k_t_layer]

            return new_layers

        elif self.factorization == 'tt':
            Gs = weight_tensor.tt_factorization(0.5)
            logger.info(f'original shape: {weight_tensor.T.shape}')
    ...
    @timeit
    def decompose_fc_layer(self, layer):
        ...
        if self.factorization == 'tt':
            weight_tensor = Tensor(weights, from_matrix=True, d=6)
            ...
            np.save('data/tt_fc_4_alexnet_cores.npy', Gs)
            tt_layer = TTLayer(in_features=[4, 4, 4, 4, 4, 4],
                    out_features=[4, 4, 4, 4, 4, 4],
                    tt_ranks=tt_ranks)
            #logger.info(weight_tensor.relative_error(tt[0].reshape(weight_tensor.T.shape)))
            return [tt_layer]
